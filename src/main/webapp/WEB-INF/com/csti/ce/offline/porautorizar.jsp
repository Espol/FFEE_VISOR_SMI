...
<html>
    ...
    <body>
        <table id="dg-porAutorizar-off" class="easyui-datagrid" style="height:475px" pagination="true" 
               data-options=" border:false, 
               url:'${pageContext.request.contextPath}/view/porautorizar/offline/load',
               striped:true,
               idField:'idDoc',
               rownumbers: true,
               selectOnCheck:true,
               checkOnSelect:true,
               singleSelect:false,
               pageSize:20" toolbar="#searchPorAutorizar-off">
        ...
        <div id="searchPorAutorizar-off" style="padding:10px 10px 10px 10px"> 
            <b>Busquedas por:</b>
...
<!--             <a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-ok'" onclick="autorizarPorAutorizar()">Autorizar</a> -->
        ...
        <script type="text/javascript" >
            ...
            var dgPorAutorizar = $("#dg-porAutorizar-off");
            dgPorAutorizar.datagrid({
                queryParams: {
                    tipoDoc: $("#tipoDocPorAutorizar").val(),
                    fechaIni: f_1_pa,
                    fechaFinal: f_td_pa,
                    nroSri: $("#searchPorAutorizar-off #nroSri").val(),
                    nroReferencia: $("#searchPorAutorizar-off #nroReferencia").val(),
                    codInterlocutor: $("#searchPorAutorizar-off #codInterlocutor").val()
                }
            });
            $('#searchPorAutorizar-off #fechaInicial').val(f_1_pa);
            $('#searchPorAutorizar-off #fechaFinal').val(f_td_pa);
            ...
            function buscarPorAutorizar() {
                $("#dg-porAutorizar-off").datagrid('reload', {
                    tipoDoc: $("#searchPorAutorizar-off input[name=tipoDocPorAutorizar]").val(),
                    fechaIni: $("#searchPorAutorizar-off #fechaInicial").datebox('getValue'),
                    fechaFinal: $("#searchPorAutorizar-off #fechaFinal").datebox('getValue'),
                    nroSri: $("#searchPorAutorizar-off #nroSri").val(),
                    nroReferencia: $("#searchPorAutorizar-off #nroReferencia").val(),
                    codInterlocutor: $("#searchPorAutorizar-off #codInterlocutor").val()
                });
            ...
            function autorizarPorAutorizar() {
                $.fn.autorizar.setData($('#dg-porAutorizar-off').datagrid('getSelections'));
                $.fn.autorizar.clearGrid = function() {
                    $('#dg-porAutorizar-off').datagrid('clearSelections');
                };
