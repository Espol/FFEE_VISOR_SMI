...
<html>
    ...
    <body>
        <table id="dg-inconsistente-off" class="easyui-datagrid" style="height:475px" pagination="true" 
               data-options="border:false,
               url:'${pageContext.request.contextPath}/view/inconsistente/offline/load',
               striped:true,
               idField:'idDoc',
               rownumbers: true,
               selectOnCheck:true,
               checkOnSelect:true,
               singleSelect:false,
               pageSize:20" toolbar="#searchInconsistente-off">
        ...
        <div id="searchInconsistente-off" style="padding:10px">
            <b>Busquedas por:</b>
        ...
        <script tabindex="text/script">
            ...
            var dgInconsistente = $("#dg-inconsistente-off");
            dgInconsistente.datagrid({
                queryParams: {
                    tipoDoc: $("#tipoDocInconsistente").val(),
                    fechaIni: f_1_i,
                    fechaFinal: f_td_i,
                    nroSri: $("#searchInconsistente-off #nroSri").val(),
                    nroReferencia: $("#searchInconsistente-off #nroReferencia").val(),
                    codInterlocutor: $("#searchInconsistente-off #codInterlocutor").val()
                }
            });
            $('#searchInconsistente-off #fechaInicial').val(f_1_i);
            $('#searchInconsistente-off #fechaFinal').val(f_td_i);
            ...
            function buscarInconsistente() {
                $("#dg-inconsistente-off").datagrid('reload', {
                    tipoDoc: $("#searchInconsistente-off input[name=tipoDocInconsistente]").val(),
                    fechaIni: $("#searchInconsistente-off #fechaInicial").datebox('getValue'),
                    fechaFinal: $("#searchInconsistente-off #fechaFinal").datebox('getValue'),
                    nroSri: $("#searchInconsistente-off #nroSri").val(),
                    nroReferencia: $("#searchInconsistente-off #nroReferencia").val(),
                    codInterlocutor: $("#searchInconsistente-off #codInterlocutor").val()
                });
