...
<html>
    ...
    <body>
        <table id="dg-contingencia" class="easyui-datagrid" style="height:475px" pagination="true" 
               data-options="border:false,
               url:'${pageContext.request.contextPath}/view/contingencia/comprobante/load',
               striped:true,
               idField:'id',
               rownumbers: true,
               selectOnCheck:true,
               checkOnSelect:true,
               singleSelect:false,
               pageSize:20" toolbar="#searchContingencia">
        ...
        <div id="searchContingencia" style="padding:10px">
            <b>Busquedas por:</b>
        ...
        <script tabindex="text/script">
            ...
            var dgContingencia = $("#dg-contingencia");
            dgContingencia.datagrid({
                queryParams: {
                    tipoDoc: $("#tipoDocContingencia").val(),
                    fechaIni: f_1_c,
                    fechaFinal: f_td_c,
                    nroSri: $("#searchContingencia #nroSri").val(),
                    nroReferencia: $("#searchContingencia #nroReferencia").val(),
                    codInterlocutor: $("#searchContingencia #codInterlocutor").val()
                }
            });
            $('#searchContingencia #fechaInicial').val(f_1_c);
            $('#searchContingencia #fechaFinal').val(f_td_c);
            ...
            function buscarContingencia() {
                $("#dg-contingencia").datagrid('reload', {
                    tipoDoc: $("#searchContingencia input[name=tipoDocContingencia]").val(),
                    fechaIni: $("#searchContingencia #fechaInicial").datebox('getValue'),
                    fechaFinal: $("#searchContingencia #fechaFinal").datebox('getValue'),
                    nroSri: $("#searchContingencia #nroSri").val(),
                    nroReferencia: $("#searchContingencia #nroReferencia").val(),
                    codInterlocutor: $("#searchContingencia #codInterlocutor").val()
                });
            ...
            function autorizarContingencia() {
                $.fn.autorizar.setData($('#dg-contingencia').datagrid('getSelections'));
                $.fn.autorizar.clearGrid=function(){
                    $('#dg-contingencia').datagrid('clearSelections');
                };
