...
<html>
    ...
    <body>
        <table id="dg-pendiente" class="easyui-datagrid" style="height:475px" pagination="true" 
               data-options="border:false,
               url:'${pageContext.request.contextPath}/view/pendiente/comprobante/load',
               striped:true,
               idField:'idDoc',
               rownumbers: true,
               selectOnCheck:true,
               checkOnSelect:true,
               singleSelect:false,
               pageSize:20" toolbar="#searchPendiente">
        ...
        <div id="searchPendiente" style="padding:10px">
            <b>Busquedas por:</b>
            ...
            <a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-search'" style="width:80px" onclick="buscarPendiente()">Buscar</a> 

        </div>
        <script tabindex="text/script">
            var f_1_i = $.fn.date.newDateFirstDay();
            var f_td_i = $.fn.date.newDate();
            var dgInconsistente = $("#dg-pendiente");
            dgInconsistente.datagrid({
                queryParams: {
                    tipoDoc: $("#tipoDocInconsistente").val(),
                    fechaIni: f_1_i,
                    fechaFinal: f_td_i,
                    nroSri: $("#searchPendiente #nroSri").val(),
                    nroReferencia: $("#searchPendiente #nroReferencia").val(),
                    codInterlocutor: $("#searchPendiente #codInterlocutor").val()
                }
            });
            $('#searchPendiente #fechaInicial').val(f_1_i);
            $('#searchPendiente #fechaFinal').val(f_td_i);
            ...
            pagerInconsistente.pagination({
             ...
             $('#searchPendiente #tipoDocInconsistente').combobox({
                  onSelect: function() {                 
                   buscarPendiente();
                  }
               });
            ...
            function buscarPendiente() {
                $("#dg-pendiente").datagrid('reload', {
                    tipoDoc: $("#searchPendiente input[name=tipoDocInconsistente]").val(),
                    fechaIni: $("#searchPendiente #fechaInicial").datebox('getValue'),
                    fechaFinal: $("#searchPendiente #fechaFinal").datebox('getValue'),
                    nroSri: $("#searchPendiente #nroSri").val(),
                    nroReferencia: $("#searchPendiente #nroReferencia").val(),
                    codInterlocutor: $("#searchPendiente #codInterlocutor").val()
                });
